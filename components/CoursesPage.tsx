...
const CoursesPage: React.FC<CoursesPageProps> = ({ courses, onSelectCourse, currency, exchangeRate, strings, language }) => {
  ...
  const filteredAndSortedCourses = useMemo(() => {
    ...
    const getPriceValue = (c: Course) => {
        let p = 0;
        if (currency === 'USD') {
            p = c.priceUsd ?? (c.priceJod ? c.priceJod / exchangeRate : (c.price ? c.price / exchangeRate : 0));
        } else if (currency === 'SAR') {
            p = c.priceSar ?? (c.priceJod ? c.priceJod * 5.3 : (c.price ? c.price * 5.3 : 0));
        } else {
            p = c.priceJod ?? c.price ?? 0;
        }
    ...
    return result;
  }, [courses, selectedCategory, selectedLevel, selectedCurriculum, selectedGrade, sortBy, currency, exchangeRate]);
