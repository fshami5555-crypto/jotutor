...
interface ManageCoursesProps {
    ...
    curriculums: string[];
    isEnglishAdmin?: boolean; // New Prop
}
...
const CourseFormModal: React.FC<{ 
    ...
    // Initialize form with either English or Arabic fields based on isEnglishAdmin
    const [formData, setFormData] = useState<Partial<Course>>({});

    // Define Grades Options
    const gradesAr = [
        'روضة 1', 'روضة 2',
    ...
    useEffect(() => {
        if (isEnglishAdmin) {
            setFormData({
                // Load existing English values or fallback to empty strings
                ...
                targetGrades: course?.targetGrades || [], // Shared field, but logically separated by value if completely disjoint
    ...
    const handleSubmit = (e: React.FormEvent) => {
        e.preventDefault();
        // Merge the new form data into the existing course object to preserve other fields
                ...
                <div className="bg-white rounded-lg shadow-xl w-full max-w-2xl max-h-[90vh] overflow-y-auto">
                    <form onSubmit={handleSubmit} className="p-6">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            {/* Only editable fields are English variants */}
                            ...
                            <div className="md:col-span-2">
                                <label className="block text-xs text-gray-500 mb-2">Target Grade Levels (Select all that apply)</label>
                                <div className="grid grid-cols-2 md:grid-cols-3 gap-2 bg-gray-50 p-2 rounded border max-h-40 overflow-y-auto">
                                    {availableGrades.map(grade => (
                                        <label key={grade} className="flex items-center space-x-2 cursor-pointer">
                                            <input 
                                                type="checkbox" 
                                                checked={formData.targetGrades?.includes(grade) || false} 
            ...
            <div className="bg-white rounded-lg shadow-xl w-full max-w-2xl max-h-[90vh] overflow-y-auto">
                <form onSubmit={handleSubmit} className="p-6">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        {/* Target Grades Selection */}
                        <div className="md:col-span-2">
                            <label className="block text-xs text-gray-500 mb-2">الصفوف المستهدفة (يمكن اختيار أكثر من صف)</label>
                            <div className="grid grid-cols-2 md:grid-cols-3 gap-2 bg-gray-50 p-2 rounded border max-h-40 overflow-y-auto">
                                {availableGrades.map(grade => (
                        ...
                        {/* Included Subjects */}
                        <div className="md:col-span-2">
                            <label className="block text-xs text-gray-500 mb-1">المواد التي تشملها الدورة (تشمل المواد التالية)</label>
                        ...
                        <div className="grid grid-cols-2 gap-2 bg-blue-50 p-2 rounded border md:col-span-2">
                             ...
                             <div>
                                <label className="block text-xs text-gray-500 mb-1">مدة الحصة (ساعة/لكل حصة)</label>
                    ...
                    <table className="min-w-full bg-white">
                        <thead className="bg-gray-100">
                            <tr>
                                ...
                                <th className="text-right py-3 px-4 font-semibold text-sm">Grades</th>
                            ...
                            {courses.map(course => (
                                <tr key={course.id} className="border-b">
                                    ...
                                    <td className="py-3 px-4 text-gray-600 text-xs max-w-xs truncate">
                                        {course.targetGrades ? course.targetGrades.join(', ') : '-'}
                                    </td>
