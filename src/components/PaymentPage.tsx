...
const PaymentPage: React.FC<PaymentPageProps> = ({ course, currency, strings, onEnroll }) => {
    ...
    // Mastercard API Config (Based on provided links)
    const API_VERSION = "72"; // Using latest stable version mentioned in docs
    ...
    /**
     * تنفيذ عملية الدفع والتحقق من متطلبات 3D Secure
     * المرجع: https://na.gateway.mastercard.com/api/documentation/apiDocumentation/rest-json/version/latest/apiReference.html
     */
    const initiateGatewayPayment = async () => {
        ...
        try {
            ...
            const payload = {
                ...
                interaction: {
                    operation: "AUTHORIZE",
                    // سيقوم النظام بإرجاع رابط الاستجابة بعد نجاح التحقق من البنك
            ...
            // الحالة 1: نجاح مباشر (بدون 3DS أو تم التحقق مسبقاً)
            if (response.ok && data.result === 'SUCCESS') {
                onEnroll(course, 'Success', { orderId, transactionId, paymentMethod: 'Credit Card' });
            } 
            // الحالة 2: البنك يطلب 3D Secure (التوجيه لصفحة البنك)
            else if (data.authentication?.redirect && data.authentication.redirectHtml) {
                setBankHtml(data.authentication.redirectHtml);
                setShowBankPage(true);
                setIsProcessing(false);
            }
            // الحالة 3: رفض العملية
            else {
                const errorDetail = data.error?.explanation || data.response?.gatewayCode || "العملية مرفوضة";
    ...
    // الاستماع لردود الـ Iframe (في حال وجود Integration JS)
    useEffect(() => {
        const handleIframeMessage = (event: MessageEvent) => {
            // منطق معالجة العودة من صفحة البنك
            if (event.data === '3DS_COMPLETED') {
                setShowBankPage(false);
                onEnroll(course, 'Success', { paymentMethod: 'Credit Card' });
            }
        };
                ...
                <div className="grid grid-cols-1 lg:grid-cols-5 gap-8">
                    ...
                    <div className="lg:col-span-2 space-y-4">
                        ...
                        <div className="flex items-center gap-2 text-xs text-gray-400 justify-center">
                            ...
                            Secure Hosted Session by Mastercard
                        </div>
                    </div>
                    ...
                    <div className="lg:col-span-3 bg-white p-8 rounded-2xl shadow-xl">
                        <div className="flex gap-4 mb-8">
                            <button onClick={() => setPaymentMethod('visa')} className={`flex-1 py-3 rounded-xl border-2 font-bold transition-all ${paymentMethod === 'visa' ? 'border-green-500 bg-green-50 text-green-700' : 'border-gray-100 text-gray-400'}`}>بطاقة بنكية</button>
                            <button onClick={() => setPaymentMethod('cliq')} className={`flex-1 py-3 rounded-xl border-2 font-bold transition-all ${paymentMethod === 'cliq' ? 'border-green-500 bg-green-50 text-green-700' : 'border-gray-100 text-gray-400'}`}>دفع محلي</button>
                        </div>
                        ...
                        <form onSubmit={handleFormSubmit} className="space-y-5">
                            {paymentMethod === 'visa' ? (
                                ...
                                <div className="bg-blue-50 p-4 rounded-xl text-blue-800 text-sm font-bold border border-blue-100">
                                    سيتم تحويلك لبيانات الدفع المحلية (كليك/زين كاش) عند التأكيد.
                                </div>
                            )}
            ...
            {showBankPage && bankHtml && (
                <div className="fixed inset-0 z-[200] bg-white flex flex-col">
                    ...
                    <div className="bg-gray-50 p-4 text-center text-[10px] text-gray-400">
                        JoTutor uses Mastercard Payment Gateway Services to ensure your data security.
                    </div>
                </div>
            )}
        </div>
