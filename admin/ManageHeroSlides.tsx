...
const SlideFormModal: React.FC<{ slide: HeroSlide | null; onSave: (slide: HeroSlide) => void; onClose: () => void; }> = ({ slide, onSave, onClose }) => {
...
const ManageHeroSlides: React.FC<ManageHeroSlidesProps> = ({ heroSlides, setHeroSlides }) => {
    ...
    return (
        <div>
            {isModalOpen && <SlideFormModal slide={editingSlide} onSave={handleSaveSlide} onClose={handleCloseModal} />}
            <div className="flex justify-between items-center mb-6">
                <h1 className="text-3xl font-bold text-gray-800">إدارة بنر الرئيسية</h1>
                    ...
                    {heroSlides.map(slide => (
                        <div key={slide.id} className="flex items-start justify-between p-4 border rounded-md bg-gray-50">
                             <div className="flex items-start space-x-4 space-x-reverse">
                                ...
                                <div>
                                    <p className="font-semibold text-blue-900">{slide.title}</p>
                                    <p className="text-gray-600 text-sm mt-1">{slide.description}</p>
                                </div>
                            </div>
                            <div className="flex-shrink-0 flex flex-col space-y-2">
                                <button onClick={() => handleOpenModal(slide)} className="text-blue-500 hover:underline text-sm text-left">تعديل</button>
                                <button onClick={() => handleRemoveSlide(slide.id)} className="text-red-500 hover:underline text-sm text-left">حذف</button>
                            </div>
                        </div>
                    ))}
                </div>
